...
var HelloWorld = class extends CustomType {
  constructor(text2) {
    super();
    this.text = text2;
  }
};
var UserEnteredSite = class extends CustomType {
};
var ApiFetchedHello = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function get_hello_world() {
  ...
  return get2(
    ...
    expect_json(
      decoder,
      (var0) => {
        return new ApiFetchedHello(var0);
      }
    )
  );
}
function update2(model, msg) {
  if (msg instanceof UserEnteredSite) {
    return [model, get_hello_world()];
  } else if (msg instanceof ApiFetchedHello && msg[0].isOk()) {
    let text2 = msg[0][0];
    return [new Model(new Some(text2)), none()];
  } else {
    return [model, none()];
  }
}
function view(model) {
  ...
  let $ = model.hello;
  if ($ instanceof Some && $[0] instanceof HelloWorld) {
    let text2 = $[0].text;
    return body(
      toList([style(styles)]),
      toList([
    ...
    return div(
      ...
      toList([
        button(
          toList([on_click(new UserEnteredSite())]),
          toList([text("Enter")])
        )
      ])
    );
